...
### Loss of Locality
如果不需要再文件或者package之间相互跳转，短小函数是非常合适的。《Clean Code》一书中提出了一个The Stepdown Rule。
> 我们希望你的代码读起来像一种自上而下设计的叙述结构。我们希望每一个函数下面都是另外一层抽象。这样的话，当我们阅读代码时，每次下降一层。我把以上原则> > 成为THE Stepdown Rule。

这个理论上看是可行的，但是我很少见到编程实践中表现良好的时候。反而，我看到的往往是当更多函数加入的时候，oss of locality。

假设我们有三个函数，调用关系 A->B->C。根据确定的假设或者需求，我们经过了仔细的研究、推论等我们设计了最初的抽象结构。
很快我们有了新的需求或者发现一个未处理的边界情况。如果把改动直接放到A函数中，那么A就不再符合“只做一件事”的原则了，所以我们只能修改A函数。根据《Clean Code》书中的原则，最好的解决办法就是抽象出几个小的函数来分解这个需求。结果：A->X->Y->B->C。
几周之后，如果需求再次变动，我们需要抽象更多函数了：A->X->Y->B->M->N->C。
重复以上步骤，我们将会遇到Sandi Metz在她的文章中提到的问题：
> 存在的代码有着重要的影响。它的存在表明名它的正确和必要性。这些代码是花费了努力写成的，我们更想让这份付出有意义。不幸的是代码越复杂、难懂，付出的努力越多，我们越想要保持这份代码(沉没成本陷阱)。

如果代码库的初始开发团队来继续维护这份代码，很可能遇到沉没成本陷阱，如果新的团队接手这份代码情况会完全不同。最开始做的层层抽象本来是好意，但是经过后面维护者的不断修改，很容易变成来回、反复调用的面条式的代码，所谓简洁不复存在。最后维护者更多的是想着重构或者重写了。

也许有人会说，某种程度来说，这是必然的。他们也许是对的。我们很少讨论如何让一段代码能够被优雅的删除。
> 如何让代码能够优雅的退役也是我们优化的方向。开闭原则(ocp，对修改关闭，对扩展开放)可以实现这点。

通常来讲，只有当程序被删除、不再被调用或者整个服务不再使用的时候，我们才认为代码“废弃”了。如果我们把代码的“废弃”看做一个过程式的，而不是突发式的，我们的每次代码提交都会让代码逐渐接近“废弃”，也许我们会更有动力去写出易于修改的代码。所以，当我们考虑如何抽象的时候，考虑下这份代码也许几个小时之后就要被修改或者废弃会更有帮助。易于修改的代码比教条的自上而下式的代码更有意义。

### 对类的坏处
如果功能拆分过细，函数过小，在oop语言中必然导致类中更多的函数，相互关系变复杂，或者导致我们需要写更多的类。在Go语言中就是更大的接口或者更多的包。
